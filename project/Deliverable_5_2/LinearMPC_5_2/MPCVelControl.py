...
from LinearMPC_5_2.MPCControl_roll import MPCControl_roll
from LinearMPC_5_2.MPCControl_xvel import MPCControl_xvel
from LinearMPC_5_2.MPCControl_yvel import MPCControl_yvel
from LinearMPC_5_2.MPCControl_zvel import MPCControl_zvel
...
class MPCVelControl:
    ...
    vel_z_traj: list
    d_estimate_traj: list

    def __init__(self) -> None:
        # Initialize trajectory lists
        self.vel_z_traj = []
        self.d_estimate_traj = []
    ...
    def save_zvel_trajectory(self, x0: np.ndarray) -> None:
        """
        Save the current z-velocity state and disturbance estimate to trajectories.
        ...
        # Get disturbance estimate (ensure it's a 1D array)
        if hasattr(self.mpc_z, 'd_estimate') and self.mpc_z.d_estimate is not None:
            d_est = self.mpc_z.d_estimate.flatten() if self.mpc_z.d_estimate.ndim > 1 else self.mpc_z.d_estimate
            self.d_estimate_traj.append(d_est.copy())
        else:
            # If d_estimate doesn't exist yet, append zeros
            self.d_estimate_traj.append(np.zeros(self.mpc_z.nu))
